...
async function apiRequest(endpoint, method = 'GET', body = null) {
    ...
    try {
        ...
        throw new Error('Cannot reach backend on port 5000. Make sure node server.js is running.');
    }
...
document.getElementById('nextTourBtn').addEventListener('click', () => {
    ...
    if (tourSlide < 3) {
        ...
        if (tourSlide === 2) document.getElementById('nextTourBtn').innerText = "Let's Get Started";
    } else {
...
const authFinanceGroup = document.getElementById('authFinanceGroup');
...
authTabLogin.addEventListener('click', () => {
    isLoginMode = true;
    authTabLogin.classList.add('bg-white', 'shadow-sm', 'text-navy');
    authTabLogin.classList.remove('bg-transparent', 'text-slate-500');
    authTabRegister.classList.add('bg-transparent', 'text-slate-500');
    authTabRegister.classList.remove('bg-white', 'shadow-sm', 'text-navy');
    authNameGroup.classList.add('d-none');
    authFinanceGroup.classList.add('d-none');
    authModalTitle.innerText = "Welcome Back";
    authModalSubtitle.innerText = "Sign in to sync your financial ledger";
    authSubmitBtn.innerText = "Sign In";
});

authTabRegister.addEventListener('click', () => {
    isLoginMode = false;
    authTabRegister.classList.add('bg-white', 'shadow-sm', 'text-navy');
    authTabRegister.classList.remove('bg-transparent', 'text-slate-500');
    authTabLogin.classList.add('bg-transparent', 'text-slate-500');
    authTabLogin.classList.remove('bg-white', 'shadow-sm', 'text-navy');
    authNameGroup.classList.remove('d-none');
    authFinanceGroup.classList.remove('d-none');
    authModalTitle.innerText = "Create Account";
    authModalSubtitle.innerText = "Join CacheFlow to access your synced cloud ledger";
    authSubmitBtn.innerText = "Create Cloud Account";
});
...
document.getElementById('authForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (!isLoginMode) {
            ...
            const salaryInput = document.getElementById('authSalary');
            const goalInput = document.getElementById('authGoal');

            payload.name = nameInput && nameInput.value.trim() ? nameInput.value.trim() : 'User';
            payload.salary = salaryInput && salaryInput.value ? parseFloat(salaryInput.value) : 0;
            payload.savingsGoal = goalInput && goalInput.value ? parseFloat(goalInput.value) : 0;
        }
        ...
        showBanner(isLoginMode ? `Welcome back, ${currentUser.name}!` : 'Account created successfully!');
        
        authModal.classList.add('d-none');
        await initDashboardView();
    } catch (err) {
        console.error('Auth Error:', err);
        showBanner(err.message || 'Connection failed. Is the backend server running?', 'error');
    } finally {
        authSubmitBtn.disabled = false;
        authSubmitBtn.innerText = isLoginMode ? "Sign In" : "Create Cloud Account";
    }
});
...
async function initDashboardView() {
    ...
    document.getElementById('welcomeMessage').innerText = currentUser.name || 'Account';
    await fetchExpenses();
}
...
const setupModal = document.getElementById('onboardingModal');
document.getElementById('editProfileBtn').addEventListener('click', (e) => {
    e.preventDefault();
    document.getElementById('initName').value = currentUser ? currentUser.name : '';
    document.getElementById('initSalary').value = currentUser ? currentUser.salary : 0;
    document.getElementById('initGoal').value = currentUser ? currentUser.savingsGoal : 0;
    
    document.getElementById('setupModalTitle').innerText = "Edit Profile & Goals";
    document.getElementById('setupModalSubtitle').innerText = "Adjust your monthly budget parameters.";
    document.getElementById('saveSetupBtn').innerText = "Save Changes";
    
    setupModal.classList.remove('d-none');
    lucide.createIcons();
});

document.getElementById('closeModalBtn').addEventListener('click', () => setupModal.classList.add('d-none'));
document.getElementById('cancelSetupBtn').addEventListener('click', () => setupModal.classList.add('d-none'));

document.getElementById('setupForm').addEventListener('submit', async (e) => {
    e.preventDefault();
    const name = document.getElementById('initName').value.trim();
    const salary = parseFloat(document.getElementById('initSalary').value) || 0;
    const savingsGoal = parseFloat(document.getElementById('initGoal').value) || 0;

    try {
        currentUser = await apiRequest('/auth/profile', 'PUT', { name, salary, savingsGoal });
        document.getElementById('welcomeMessage').innerText = currentUser.name;
        setupModal.classList.add('d-none');
...
document.getElementById('expenseForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        showBanner(`<strong>${formatINR(amount)}</strong> logged for ${cat}.`);
...
document.getElementById('confirmExportBtn').addEventListener('click', () => {
    ...
    document.getElementById('printUserHolder').innerText = `User: ${currentUser ? currentUser.name : 'Client Account'}`;
    const now = new Date();
...
function calculateStreak() {
    ...
    if (expenses.length === 0) {
        badge.innerHTML = `<i data-lucide="flame" class="text-brand-orange" style="width:14px;height:14px;"></i> <span>0 Day Streak</span>`;
        lucide.createIcons();
        return;
    }
    ...
    badge.innerHTML = `<i data-lucide="flame" class="text-brand-orange" style="width:14px;height:14px;"></i> <span>${streak} Day Streak</span>`;
    lucide.createIcons();
}
...
function updateDashboard(dataArray = expenses, isFiltered = false) {
            ...
            tbody.innerHTML += `
                <tr class="row-enter" style="animation-delay: ${delay}s;">
                    ...
                    <td class="text-end pe-4 no-print">
                        <button class="btn btn-sm text-slate-400 hover:text-danger border-0 delete-tx-btn transition-base p-1 bg-transparent" data-id="${exp._id}" title="Delete">
    ...
    const userSalary = currentUser ? currentUser.salary : 0;
    const savingsGoal = currentUser ? currentUser.savingsGoal : 0;
